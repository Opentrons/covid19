...
def run(ctx: protocol_api.ProtocolContext):
    ...
    for m in mag_samples_m:
        pick_up(m300)
        m300.transfer(50, enzyme_mix, m, new_tip='never')
        m300.aspirate(20, m.top())
        m300.drop_tip()
    ...
    for vol, wash_reagent in zip([1000, 1000, 500], [wash_buffer, etoh, etoh]):
        ...
        for i, m in enumerate(mag_samples_m):
            ...
            for _ in range(num_trans):
                m300.transfer(
                    vol_per_trans, wash_reagent, m.top(), new_tip='never')
            m300.move_to(m.center())
            m300.mix(10, 200, loc)
            m300.blow_out(m.top(-2))
            m300.aspirate(20, m.top(-2))
