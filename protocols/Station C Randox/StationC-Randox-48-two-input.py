...
def run(protocol):
    ...
    elution_plate = protocol.load_labware(
        'nest_96_wellplate_100ul_pcr_full_skirt', '1', 'Elution Plate 1')

    ep2 = protocol.load_labware(
        'nest_96_wellplate_100ul_pcr_full_skirt', '2', 'Elution Plate 2')
    ...
    half_samp = int(SAMPLE_NUMBER/2)
    sample_wells = elution_plate.wells()[:half_samp] + ep2.wells()[:half_samp]

    # Distribute Reaction Mix
    protocol.comment('Distributing master mix to 24 wells:')
    p20.pick_up_tip()
    ...
    for idx, (src, dest) in enumerate(zip(sample_wells, tempplate_wells)):
        p20.pick_up_tip()
        x = idx+1 if idx < 24 else idx-23
        y = 1 if idx < 24 else 2
        protocol.comment(f'Mixing sample {x} on plate {y}:')
        p20.aspirate(1, src)
        for _ in range(2):
            p20.aspirate(19, src)
            p20.dispense(19, src)

        protocol.comment(f'Transferring sample {x} from plate {y}:')
        p20.aspirate(4, src)
        p20.dispense(4, dest)
