...
metadata = {
    'protocolName': 'BP Genomics RNA Extraction 48 samples',
...
def run(protocol):
    ...
    def wash_step_rescue(src, mtimes, tips, wasteman):
        # p300.pick_up_tip(tips[0])
        # if src == wb1:
        #     for well, s in zip(magsamps, src):
        #         for _ in range(3):
        ...
        #for well, tip in zip(magsamps, tips):
        for well, tip in list(zip(magsamps, tips))[1:]:
            if not p300.hw_pipette['has_tip']:
                p300.pick_up_tip(tip)
            well_mix(mtimes, well, 180)
            p300.blow_out(well.top(-3))
        ...
        for well, tip in zip(magsamps, tips): # Rescue note: this means the old tip rack must be full, if it's being reused.
            p300.pick_up_tip(tip)
            supernatant_removal(600, well, wasteman)
            p300.return_tip()
    ...
    wash_step_rescue(wb1, 20, tips3, waste2)
